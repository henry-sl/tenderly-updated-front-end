...
// Updated layout component to use the new responsive sidebar with context provider
...
export default function Layout({ children }) {
  ...
  return (
    <SidebarProvider persistState={true}>
      <div className="min-h-screen bg-gray-50 flex">
        {/* Sidebar Navigation - hidden on homepage */}
        {!isHomepage && <Sidebar />}
        
        {/* Main Content Area */}
        <div className={`flex-1 flex flex-col min-w-0 ${isHomepage ? 'w-full' : ''}`}>
          {/* Main content where page-specific content is rendered */}
          <main className={`flex-1 ${isHomepage ? 'p-0' : 'p-4 lg:p-8'}`}>
            {children}
          </main>
          
          {/* Footer with attribution - only show on non-homepage */}
          {!isHomepage && (
